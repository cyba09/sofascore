...
response = requests.get('https://www.sofascore.com/api/v1/team/218259/events/last/0', cookies=cookies, headers=headers).json()
for game in response['events']:
    try:
        print(game['winnerCode'])
        print(game['startTimestamp'])
        print('############')
    except:
        continue